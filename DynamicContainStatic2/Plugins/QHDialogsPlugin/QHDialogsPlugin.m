...
@implementation QHDialogsPlugin
...
- (void)showDialogWithMessage:(NSString*)message title:(NSString*)title buttons:(NSArray*)buttons defaultText:(NSString*)defaultText callbackId:(NSString*)callbackId dialogType:(NSString*)dialogType
{
    ...
    if (NSClassFromString(@"UIAlertController")) {
        ...
        __weak QHDialogsPlugin* weakNotif = self;
        ...
        for (int n = 0; n < count; n++) {
                                                              ...
                                                              handler:^(UIAlertAction * action)
            {
                ...
                if ([dialogType isEqualToString:DIALOG_TYPE_PROMPT])
                {
                    ...
                    result = [QHPluginResult resultWithStatus:QHCommandStatus_OK messageAsDictionary:info];
                }
                else
                {
                    result = [QHPluginResult resultWithStatus:QHCommandStatus_OK messageAsInt:(int)(n  + 1)];
                }
    ...
    else
    {
        ...
        QHAlertView* alertView = [[QHAlertView alloc]
                                   initWithTitle:title
                                   message:message
                                   delegate:self
...
- (void)alert:(QHInvokedUrlCommand*)command
{
...
- (void)confirm:(QHInvokedUrlCommand*)command
{
    NSString* callbackId = command.callbackId;
    NSString* message = [command argumentAtIndex:0];
    NSString* title = [command argumentAtIndex:1];
...
- (void)prompt:(QHInvokedUrlCommand*)command
{
    NSString* callbackId = command.callbackId;
    NSString* message = [command argumentAtIndex:0];
    NSString* title = [command argumentAtIndex:1];
...
{
    QHAlertView* qhAlertView = (QHAlertView*)alertView;
    QHPluginResult* result;
    ...
    if (alertView.alertViewStyle == UIAlertViewStyleDefault) {
        ...
        result = [QHPluginResult resultWithStatus:QHCommandStatus_OK messageAsInt:(int)(buttonIndex + 1)];
    } else {
    ...
    [self.commandDelegate sendPluginResult:result callbackId:qhAlertView.callbackId];
}
...
- (void)beep:(QHInvokedUrlCommand*)command
{
