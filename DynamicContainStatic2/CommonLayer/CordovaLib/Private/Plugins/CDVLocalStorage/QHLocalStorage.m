...
@interface QHLocalStorage ()

@property (nonatomic, readwrite, strong) NSMutableArray* backupInfo;  // array of QHBackupInfo objects
@property (nonatomic, readwrite, weak) id <UIWebViewDelegate> webviewDelegate;

@end

@implementation QHLocalStorage
...
+ (NSMutableArray*)createBackupInfoWithTargetDir:(NSString*)targetDir backupDir:(NSString*)backupDir targetDirNests:(BOOL)targetDirNests backupDirNests:(BOOL)backupDirNests rename:(BOOL)rename
{
    ...
    backupItem = [[QHBackupInfo alloc] init];
...
+ (BOOL)copyFrom:(NSString*)src to:(NSString*)dest error:(NSError* __autoreleasing*)error
{
    ...
    if (![fileManager fileExistsAtPath:src]) {
        ...
        if (error != NULL) {
            (*error) = [NSError errorWithDomain:kQHLocalStorageErrorDomain
                                           code:kQHLocalStorageFileOperationError
...
- (BOOL)shouldBackup
{
    for (QHBackupInfo* info in self.backupInfo) {
...
- (void)backup:(QHInvokedUrlCommand*)command
{
        ...
        if ([info shouldBackup]) {
            ...
            if (callbackId) {
                if (error == nil) {
                    ...
                    result = [QHPluginResult resultWithStatus:QHCommandStatus_OK messageAsString:message];
                    [self.commandDelegate sendPluginResult:result callbackId:callbackId];
                } else {
                    message = [NSString stringWithFormat:@"Error in QHLocalStorage (%@) backup: %@", info.label, [error localizedDescription]];
                    NSLog(@"%@", message);

                    result = [QHPluginResult resultWithStatus:QHCommandStatus_ERROR messageAsString:message];
                    [self.commandDelegate sendPluginResult:result callbackId:callbackId];
                }
            }
        }
...
- (void)restore:(QHInvokedUrlCommand*)command
{
    NSError* __autoreleasing error = nil;
    QHPluginResult* result = nil;
    NSString* message = nil;
    ...
    for (QHBackupInfo* info in self.backupInfo) {
        if ([info shouldRestore]) {
            ...
            if (error == nil) {
                ...
                message = [NSString stringWithFormat:@"Error in QHLocalStorage (%@) restore: %@", info.label, [error localizedDescription]];
                NSLog(@"%@", message);
...
+ (void)__restoreLegacyDatabaseLocationsWithBackupType:(NSString*)backupType
{
    ...
    if ([backupType isEqualToString:@"cloud"]) {
            ...
            NSLog(@"\n\nStarted backup to iCloud! Please be careful."
                ...
                "\nTo disable web storage backup to iCloud, set the BackupWebStorage preference to \"local\" in the Cordova config_loan.xml file\n\n");
#endif
    ...
    for (QHBackupInfo* info in backupInfo) {
...
- (void)onResignActive
{
    ...
    if (exitsOnSuspend) {
        ...
        QHLocalStorage __weak* weakSelf = self;
...
#pragma mark -
#pragma mark QHBackupInfo implementation

@implementation QHBackupInfo
