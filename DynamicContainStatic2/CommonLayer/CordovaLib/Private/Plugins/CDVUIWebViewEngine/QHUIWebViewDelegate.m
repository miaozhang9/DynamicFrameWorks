...
@implementation QHUIWebViewDelegate
        ...
        if (isTopLevelNavigation) {
            ...
            switch (_state) {
                case STATE_WAITING_FOR_LOAD_FINISH:
                    ...
                    if (_loadCount != 1) {
                        NSLog(@"QHWebViewDelegate: Detected redirect when loadCount=%ld", (long)_loadCount);
                    }
                    break;
                ...
                default:
                    {
                        NSString* description = [NSString stringWithFormat:@"QHWebViewDelegate: Navigation started when state=%ld", (long)_state];
                        NSLog(@"%@", description);
                        _loadCount = 0;
                        _state = STATE_WAITING_FOR_LOAD_START;
                        if ([_delegate respondsToSelector:@selector(webView:didFailLoadWithError:)]) {
                            NSDictionary* errorDictionary = @{NSLocalizedDescriptionKey : description};
                            NSError* error = [[NSError alloc] initWithDomain:@"QHUIWebViewDelegate" code:1 userInfo:errorDictionary];
...
- (void)webViewDidStartLoad:(UIWebView*)webView
{
    ...
    switch (_state) {
        ...
        case STATE_WAITING_FOR_LOAD_START:
            if (_loadCount != 0) {
                NSLog(@"QHWebViewDelegate: Unexpected loadCount in didStart. count=%ld", (long)_loadCount);
            }
            fireCallback = YES;
            _state = STATE_WAITING_FOR_LOAD_FINISH;
            _loadCount = 1;
        ...
        default:
            NSLog(@"QHWebViewDelegate: Unexpected didStart with state=%ld loadCount=%ld", (long)_state, (long)_loadCount);
    }
...
- (void)webViewDidFinishLoad:(UIWebView*)webView
{
    ...
    switch (_state) {
        ...
        case STATE_WAITING_FOR_LOAD_START:
            NSLog(@"QHWebViewDelegate: Unexpected didFinish while waiting for load start.");
            break;
...
- (void)webView:(UIWebView*)webView didFailLoadWithError:(NSError*)error
{
    ...
    if (fireCallback && [_delegate respondsToSelector:@selector(webView:didFailLoadWithError:)]) {
        ...
        [QHUIWebViewDelegate cancelPreviousPerformRequestsWithTarget:self selector:@selector(addFailViewIntoWebView:) object:webView];
        [self performSelector:@selector(addFailViewIntoWebView:) withObject:webView afterDelay:.05];
    }
}

// 失败页 -- gpw

- (void)addFailViewIntoWebView:(UIWebView *)webView
{
    if (_isFail) {
        self.failView.frame = webView.bounds;
...
- (QHNetFailView *)failView{
    if (!_failView) {
        _failView = [[QHNetFailView alloc] initWithFrame:CGRectZero withFailViewType:QHNetFailViewTypeDynamic];
