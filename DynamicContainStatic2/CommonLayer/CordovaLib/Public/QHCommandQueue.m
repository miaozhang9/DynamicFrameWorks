...
@interface QHCommandQueue () {
    NSInteger _lastCommandQueueFlushRequestId;
    __weak QHViewController* _viewController;
    NSMutableArray* _queue;
    NSTimeInterval _startExecutionTime;
}
@end

@implementation QHCommandQueue
...
- (id)initWithViewController:(QHViewController*)viewController
{
    self = [super init];
    if (self != nil) {
...
- (void)enqueueCommandBatch:(NSString*)batchJSON
{
    if ([batchJSON length] > 0) {
        ...
        if ([batchJSON length] < JSON_SIZE_FOR_MAIN_THREAD) {
            [commandBatchHolder addObject:[batchJSON qh_JSONObject]];
        } else {
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^() {
                NSMutableArray* result = [batchJSON qh_JSONObject];
...
{
    __weak QHCommandQueue* weakSelf = self;
    ...
    [_viewController.webViewEngine evaluateJavaScript:js
        ...
        if ((error == nil) && [obj isKindOfClass:[NSString class]]) {
            ...
            QH_EXEC_LOG(@"Exec: Flushed JS->native queue (hadCommands=%d).", [queuedCommandsJSON length] > 0);
        ...
        while ([_queue count] > 0) {
            ...
            while ([commandBatch count] > 0) {
                @autoreleasepool {
                    ...
                    NSArray* jsonEntry = [commandBatch qh_dequeue];
                    if ([commandBatch count] == 0) {
                        [_queue removeObjectAtIndex:0];
                    }
                    QHInvokedUrlCommand* command = [QHInvokedUrlCommand commandFromJson:jsonEntry];
                    QH_EXEC_LOG(@"Exec(%@): Calling %@.%@", command.callbackId, command.className, command.methodName);

                    if (![self execute:command]) {
#ifdef DEBUG
                            NSString* commandJson = [jsonEntry qh_JSONString];
...
- (BOOL)execute:(QHInvokedUrlCommand*)command
{
    ...
    QHPlugin* obj = [_viewController.commandDelegate getCommandInstance:command.className];

    if (!([obj isKindOfClass:[QHPlugin class]])) {
        NSLog(@"ERROR: Plugin '%@' not found, or is not a QHPlugin. Check your plugin mapping in config_loan.xml.", command.className);
        return NO;
    }
    BOOL retVal = YES;
