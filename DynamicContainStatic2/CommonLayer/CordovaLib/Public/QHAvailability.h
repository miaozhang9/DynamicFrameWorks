...
#define QH_VERSION [NSString stringWithFormat:@"%d.%d.%d", \
    (CORDOVA_VERSION_MIN_REQUIRED / 10000),                 \
    (CORDOVA_VERSION_MIN_REQUIRED % 10000) / 100,           \
    (CORDOVA_VERSION_MIN_REQUIRED % 10000) % 100]

// Enable this to log all exec() calls.
#define QH_ENABLE_EXEC_LOGGING 0
#if QH_ENABLE_EXEC_LOGGING
    #define QH_EXEC_LOG NSLog
#else
    #define QH_EXEC_LOG(...) do { \
} while (NO)
#endif
