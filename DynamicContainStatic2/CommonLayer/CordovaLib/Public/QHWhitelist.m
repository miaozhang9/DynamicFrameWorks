...
NSString* const kQHDefaultWhitelistRejectionString = @"ERROR whitelist rejection: url='%@'";
NSString* const kQHDefaultSchemeName = @"cdv-default-scheme";

@interface QHWhitelistPattern : NSObject {
    @private
...
@implementation QHWhitelistPattern
...
- (id)initWithScheme:(NSString*)scheme host:(NSString*)host port:(NSString*)port path:(NSString*)path
{
    ...
    if (self) {
        if ((scheme == nil) || [scheme isEqualToString:@"*"]) {
            ...
            _scheme = [NSRegularExpression regularExpressionWithPattern:[QHWhitelistPattern regexFromPattern:scheme allowWildcards:NO] options:NSRegularExpressionCaseInsensitive error:nil];
        }
        if ([host isEqualToString:@"*"] || host == nil) {
            _host = nil;
        } else if ([host hasPrefix:@"*."]) {
            _host = [NSRegularExpression regularExpressionWithPattern:[NSString stringWithFormat:@"([a-z0-9.-]*\\.)?%@", [QHWhitelistPattern regexFromPattern:[host substringFromIndex:2] allowWildcards:false]] options:NSRegularExpressionCaseInsensitive error:nil];
        } else {
            _host = [NSRegularExpression regularExpressionWithPattern:[QHWhitelistPattern regexFromPattern:host allowWildcards:NO] options:NSRegularExpressionCaseInsensitive error:nil];
        }
        ...
        if ((path == nil) || [path isEqualToString:@"/*"]) {
            ...
            _path = [NSRegularExpression regularExpressionWithPattern:[QHWhitelistPattern regexFromPattern:path allowWildcards:YES] options:0 error:nil];
        }
    }
    return self;
}
...
- (id)initWithArray:(NSArray*)array
{
    ...
    if (self) {
        ...
        self.whitelistRejectionFormatString = kQHDefaultWhitelistRejectionString;
    ...
    if ([origin isEqualToString:@"*"]) {
        ...
        if (m != nil) {
            ...
            if (scheme == nil) {
                ...
                [self.whitelist addObject:[[QHWhitelistPattern alloc] initWithScheme:@"http" host:host port:port path:path]];
                [self.whitelist addObject:[[QHWhitelistPattern alloc] initWithScheme:@"https" host:host port:port path:path]];
            } else {
                [self.whitelist addObject:[[QHWhitelistPattern alloc] initWithScheme:scheme host:host port:port path:path]];
            }
...
- (BOOL)URLIsAllowed:(NSURL*)url logFailure:(BOOL)logFailure
{
    ...
    // http[s] and ftp[s] should also validate against the common set in the kQHDefaultSchemeName list
    if ([scheme isEqualToString:@"http"] || [scheme isEqualToString:@"https"] || [scheme isEqualToString:@"ftp"] || [scheme isEqualToString:@"ftps"]) {
        NSURL* newUrl = [NSURL URLWithString:[NSString stringWithFormat:@"%@://%@%@", kQHDefaultSchemeName, [url host], [[url path] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
    ...
    for (QHWhitelistPattern* p in self.whitelist) {
        if ([p matches:url]) {
            return YES;
        }
    }
