...
@implementation QHInvokedUrlCommand
...
+ (QHInvokedUrlCommand*)commandFromJson:(NSArray*)jsonEntry
{
    return [[QHInvokedUrlCommand alloc] initFromJson:jsonEntry];
}
...
- (void)massageArguments
{
    ...
    for (NSUInteger i = 0, count = [_arguments count]; i < count; ++i) {
        ...
        NSString* type = [dict objectForKey:@"QHType"];
