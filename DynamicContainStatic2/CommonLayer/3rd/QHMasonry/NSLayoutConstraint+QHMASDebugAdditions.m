//
//  NSLayoutConstraint+QHMASDebugAdditions.m
//  Masonry
//
...
@implementation NSLayoutConstraint (QHMASDebugAdditions)
...
#if TARGET_OS_IPHONE || TARGET_OS_TV
        descriptionMap = @{
            @(QHMASLayoutPriorityDefaultHigh)      : @"high",
            @(QHMASLayoutPriorityDefaultLow)       : @"low",
            @(QHMASLayoutPriorityDefaultMedium)    : @"medium",
            @(QHMASLayoutPriorityRequired)         : @"required",
            @(QHMASLayoutPriorityFittingSizeLevel) : @"fitting size",
        };
#elif TARGET_OS_MAC
        descriptionMap = @{
            @(QHMASLayoutPriorityDefaultHigh)                 : @"high",
            @(QHMASLayoutPriorityDragThatCanResizeWindow)     : @"drag can resize window",
            @(QHMASLayoutPriorityDefaultMedium)               : @"medium",
            @(QHMASLayoutPriorityWindowSizeStayPut)           : @"window size stay put",
            @(QHMASLayoutPriorityDragThatCannotResizeWindow)  : @"drag cannot resize window",
            @(QHMASLayoutPriorityDefaultLow)                  : @"low",
            @(QHMASLayoutPriorityFittingSizeCompression)      : @"fitting size",
            @(QHMASLayoutPriorityRequired)                    : @"required",
        };
#endif
    });
...
+ (NSString *)descriptionForObject:(id)obj {
    if ([obj respondsToSelector:@selector(qhMas_key)] && [obj qhMas_key]) {
        return [NSString stringWithFormat:@"%@:%@", [obj class], [obj qhMas_key]];
    }
...
- (NSString *)description {
    ...
    if (self.priority != QHMASLayoutPriorityRequired) {
